...
class ScalpingStrategy:
    ...
    def run_monte_carlo_simulation(self, iterations):
        ...
        for _ in range(iterations):
            WINDOW = 200
            POSITION_SIZE = 1
            Z_SCORE = np.random.uniform(2.0, 3.5)
...
data_15m = data_15m.tail(500)


scalping_15m = ScalpingStrategy(data_15m)
for market in scalping_15m.price_data:
    # Skip 'time' or any non-market column if present
    if market == 'time':
        continue
    scalping_15m.calculate_zscore(market)
scalping_15m.run_monte_carlo_simulation(10)

# data_1H = pd.read_csv('data_test_1H.csv')


# scalping_1H = ScalpingStrategy(data_1H)
